...
```mermaid
flowchart LR
  Baseline["Known-good .NET 8<br/>build and tests"] --> Framework["Change target frameworks"]
  Framework --> Packages["Update compatible packages"]
  Packages --> APIs["Replace obsolete APIs"]
  APIs --> Restore["Restore"]
  Restore --> Build["Build"]
  Build --> Test["Test"]
  Test -->|Failure| Diagnose["Fix one compatibility issue"]
  Diagnose --> Restore
  Test -->|Pass| Net10["Known-good .NET 10 baseline"]
```
