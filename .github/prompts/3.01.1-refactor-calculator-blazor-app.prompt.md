...
## Deferred Infrastructure Context

This prompt assumes local iterative development uses a persistent PostgreSQL Docker
container seeded from `TestCases.csv` so app and UI settings remain stable between
sessions.

Testcontainers remains the preferred automated test isolation method, and local
persistent schema/data behavior should be mapped into the subsequent Azure
Database for PostgreSQL migration plan.
...
The current repository starts from `src/workspace/calculator-xunit-testing/`. The Blazor project, library project, CSV `TestCases.csv`, and implementation completion report listed below are target-state artifacts for this refactor prompt, not current files.
...
PRD Location: No active PRD file exists in the current repository; treat this prompt as the source of requirements until a tracked PRD is added.
