---
agent: agent
description: Translate a basic workflow at "$(git rev-parse --show-toplevel)\.github\workflows\01-basic-triggers-workflow.yml" from GitHub Actions to Azure DevOps Pipelines
name: 8.01.translate-workflow-to-pipeline
...
Translate the basic workflow at: "$(git rev-parse --show-toplevel)\.github\workflows\01-basic-triggers-workflow.yml" to an equivalent azure devops pipeline YAML and save it in the directory "$(git rev-parse --show-toplevel)\.azure-pipelines\01-basic-triggers-pipeline.yml".
...
- If the `01-basic-triggers-pipeline.yml` file already exists, overwrite it with the new translation.
...
- Pipeline YAML is translated and saved to `.azure-pipelines/01-basic-triggers-pipeline.yml`.
