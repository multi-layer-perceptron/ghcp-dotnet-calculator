...
4. Run the web app and the test suite:
  ...
  In Codespaces, open port 5000 from the **Ports** panel. Browser-based
  learners use the forwarded URL, while Playwright running inside the
  Codespace uses `http://localhost:5000`. Stop the app with `Ctrl+C` before
  running the test command in the same terminal, or run tests in a second
  terminal.
